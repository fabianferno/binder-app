...
contract RelationshipRegistryMock is SismoConnect {
  ...
  function validatezkProofs(bytes[2] memory zkProofs, bytes16[2] memory groupIds, address[2] memory parents) public {
    for (uint256 i = 0; i < 2; i++) {
      require(validatezkProof(zkProofs[i], groupIds[i], parents[i]), "Invalid zkProof");
    }
  }

  function validatezkProof(bytes memory zkProof, bytes16 groupId, address parent) public returns (bool) {
  ...
  function checkSignatures(bytes[2] memory signatures, bytes32 dataHash, address[2] memory parents) public {
    for (uint256 i = 0; i < 2; i++) {
  ...
  function checkSignature(bytes memory signature, bytes32 dataHash, address parent) public returns (bool) {
    bool result = _recoverSigner(_getEthSignedMessageHash(dataHash), signature) ==
      IERC6551Account(payable(parent)).owner();
    emit SignatureChecked(result);
    return result;
  }
